...
interface IPickerOnlyImg {
    selectImage?: (path: string, appendBody: IUploadImageFile) => void;
    children?: ReactNode
    onPress?: () => void
}
const PickerOnlyImg = React.memo((props: IPickerOnlyImg) => {
    // !State
    const { selectImage, children, onPress } = props;
    ...
    const handleItem = useCallback((type: string) => {
        if (type === ChooseImg.TakePhoto) {
            ImagePicker.openCamera({
                ...
                // selectImage?.(image?.path, `data:${image.mime};base64,${image.data}`);
            ...
            ImagePicker.openPicker({
                ...
                // selectImage && selectImage(image?.path, `data:${image.mime};base64,${image.data}`);
                selectImage?.(image?.path, { uri: image?.path ?? '', type: image?.mime, name: 'name' });
                modalRef.current?.close();
            });
        }
    ...
    const renderItem = (icon: any, text: string, type: string) => {
        return (
            <TouchableOpacity activeOpacity={1} onPress={() => handleItem(type)} >
